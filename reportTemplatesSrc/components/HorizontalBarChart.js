...
module.exports = props => {
  ...
  const { title, barMaxWidth = 210, captionWidth = 120 } = props;
  return (
    <React.Fragment>
      <div className="barchart-container">
        <div
          className="barchart-title"
          style={{ marginLeft: captionWidth + 10 }}
        >
          {title}
        </div>
        {data.map((entry, index) => (
          <div key={index} className="barchart-bar-and-caption">
            <div className="barchart-caption" style={{ width: captionWidth }}>
              {entry.label}
            </div>
                ...
                style={{
                  ...
                  background:
                    maxValue <= 0
                      ? null
                      : index % 2 === 0
                      ? horizontalBarColors[0]
                      : horizontalBarColors[1]
                }}
              />
            </div>
            <div className="barchart-value">
              {entry.value} ({Math.round((entry.value / maxValue) * 100)}%)
            </div>
          </div>
        ))}
      </div>
