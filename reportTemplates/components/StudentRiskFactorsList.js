...
const Item = ({ label, value }) => React.createElement(
  'div',
...
module.exports = ({ title, risk, data }) => React.createElement(
  'div',
  { className: 'risk-list' },
  React.createElement(
    'div',
  ...
  data.map((entry, index) => React.createElement(Item, { key: entry.label, index: index, label: entry.label, value: entry.value })),
  React.createElement(
    'style',
    null,
    studentRiskFactorsListStyle
  )
);