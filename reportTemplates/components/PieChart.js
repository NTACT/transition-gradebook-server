...
const addColorScheme = list => list.map((item, index) => _extends({}, item, {
...
module.exports = function PieChartWrapper(props) {
  ...
  return React.createElement(
    ...
    React.createElement(
      'div',
      { className: 'piechart-container', style: _extends({}, width) },
      React.createElement(
        'div',
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          data.sort(keySortFn).map((entry, i) => React.createElement(
            'div',
            { key: entry.label, className: 'piechart-caption-item' },
            React.createElement(
              'span',
              { className: 'piechart-label' },
              React.createElement('div', { className: 'piechart-label-icon', style: { backgroundColor: pieChartColors[i] } }),
              entry.label
            ),
            React.createElement(
              'span',
              { className: 'piechart-value' },
              isRatio && Math.round(entry.value * 100) || isPercentage && Math.round(entry.value) || isAbsolute && total > 0 && Math.round(entry.value / total * 100) || 0,
              '%'
            )
          ))
        ),
