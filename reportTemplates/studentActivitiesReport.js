...
let StudentActivitiesReport = class StudentActivitiesReport extends Component {
  render() {
    ...
    return React.createElement(
      ...
      activityTypeGroups.map((group, i) => React.createElement(
        ActivityTable,
        { key: group.id, 'break': i === 1 || i === 3 },
        React.createElement(ActivityTableHeader, { title: group.name, terms: terms }),
        React.createElement(
          'tbody',
          { style: { borderTop: '10px solid white' } },
          group.activityTypes.map((type, i) => React.createElement(
            'tr',
            { key: type.id, style: {
            ...
            React.createElement(
              'td',
              { style: { textAlign: 'left', fontSize: 10, paddingLeft: 10 } },
              type.name
            ),
            terms.map(term => React.createElement(
              'td',
              { key: term.id, style: { fontSize: 8, borderLeft: '1px solid #D43425' } },
...
const ActivityTable = props => React.createElement('table', _extends({}, props, { style: {
    width: '100%',
    textAlign: 'center',
    borderSpacing: 0,
...
const ActivityTableHeader = props => React.createElement(
  'thead',
  { style: {
      display: 'flex',
      flexDirection: 'row',
  ...
  React.createElement(
    ...
    props.terms.map((term, i) => React.createElement(
      'td',
      { key: i },
      term.name
    ))
